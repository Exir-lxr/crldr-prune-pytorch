...
class InfoStruct(object):
    ...
    def compute_statistic_and_fetch_weight(self):
        ...
        self.zero_variance_masked_zero = torch.sign(torch.diag(self.forward_cov))

        # where 0 var compensate 1
        repaired_forward_cov = self.forward_cov + \
            torch.diag(torch.nn.Parameter(torch.ones(self.channel_num, dtype=torch.double)).cuda() - \
